...
# ARuCoの追跡 + Odometryの計算
class Test2(Node):

    def __init__(self):
        super().__init__('test2')
        self.declare_parameter("aruco_topic", "/aruco_markers")
        self.declare_parameter("flight_data", "/flight_data")

        aruco_topic = self.get_parameter("aruco_topic").get_parameter_value().string_value
        flight_data_topic = self.get_parameter("flight_data").get_parameter_value().string_value
        ...
        self.flight_data_sub = self.create_subscription(FlightData,
                                                 flight_data_topic,
